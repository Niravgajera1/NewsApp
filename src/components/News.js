...
const News = (props) => {
    ...
    const fetchMoreData = async () => {
        const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apikey=6adcafee1afb43948d644977bfde06f6&page=${page + 1}&pageSize=${props.pageSize}`;
        setpage(page + 1) 
        let data = await fetch(url);
        let parsedData = await data.json()
    ...
    return (
        <>
            <div className="container my-4" style={{ maxHeight: "400px" }}>
                <h1 className='text-center' style={{ margin: '30px 0px', marginTop: '90px', color: 'black' }}><strong>Top Headlines From {capitalizeFirstLetter(props.category)} </strong></h1>
                {loading && <Spinner />}
                <InfiniteScroll
